...
import { ArrowLeft, ArrowsLeftRight, Check, Copy, FileCode } from "@phosphor-icons/react";
...
export function DocumentConverter({ onBack }: Props) {
  ...
  return <section className="tool-view"><button className="back-button" onClick={onBack}><ArrowLeft /> Volver a herramientas</button>
    ...
    <div className="document-editors"><label><span>Contenido {labels[source]}</span><textarea value={content} onChange={(event) => setContent(event.target.value)} spellCheck={false} /></label><label><span><b>Resultado {labels[target]}</b><button disabled={!result} onClick={() => void copy()}>{copied ? <Check /> : <Copy />}{copied ? "Copiado" : "Copiar"}</button></span><textarea value={result} readOnly spellCheck={false} /></label></div>
